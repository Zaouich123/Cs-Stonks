...
export class PrismaItemReadRepository implements ItemReadRepository {
  ...
  async listHistoryByItem(input: {
    ...
    return rows.map((row) => ({
      ...
      meanPrice: row.meanPrice?.toNumber() ?? null,
      medianPrice: row.medianPrice?.toNumber() ?? null,
      minPrice: row.minPrice?.toNumber() ?? null,
      price: row.price.toNumber(),
      quantity: row.quantity,
      sales24hMedian: row.sales24hMedian?.toNumber() ?? null,
      sales24hMin: row.sales24hMin?.toNumber() ?? null,
      sales24hVolume: row.sales24hVolume,
      sales30dMedian: row.sales30dMedian?.toNumber() ?? null,
      sales30dMin: row.sales30dMin?.toNumber() ?? null,
      sales30dVolume: row.sales30dVolume,
      sales7dMedian: row.sales7dMedian?.toNumber() ?? null,
      sales7dMin: row.sales7dMin?.toNumber() ?? null,
      sales7dVolume: row.sales7dVolume,
      sales90dMedian: row.sales90dMedian?.toNumber() ?? null,
      sales90dMin: row.sales90dMin?.toNumber() ?? null,
      sales90dVolume: row.sales90dVolume,
      snapshotDate: row.snapshotDate,
      snapshotHour: row.snapshotHour,
      sourceFetchedAt: row.sourceFetchedAt,
      sourceUpdatedAt: row.sourceUpdatedAt,
      suggestedPrice: row.suggestedPrice?.toNumber() ?? null,
      volume: row.volume,
    }));
  }
}
