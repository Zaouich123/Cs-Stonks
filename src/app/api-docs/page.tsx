...
import {
  localizeApiDocEndpoints,
  localizeApiDocSections,
  localizeApiStatusReference,
} from "@/lib/api-docs/api-docs-i18n";
...
function OverviewPanel() {
  ...
  const sections = localizeApiDocSections(apiDocSections, language);

  return (
    <div className="space-y-6">
      <section className="rounded-[2rem] border border-white/10 bg-[#071123]/72 p-6 shadow-[0_20px_60px_rgba(0,0,0,0.34)] md:p-8">
        <p className="text-[11px] uppercase tracking-[0.24em] text-[#7eb7ff]">{sections[0]?.eyebrow}</p>
        <h2 className="mt-4 text-3xl font-semibold text-white">{sections[0]?.title}</h2>
        <p className="mt-4 max-w-3xl text-base leading-8 text-white/66">{sections[0]?.summary}</p>

        <div className="mt-8 grid gap-4 md:grid-cols-2">
          <div className="rounded-[1.5rem] border border-white/10 bg-white/[0.03] p-5">
            <p className="text-xs uppercase tracking-[0.22em] text-white/45">{t("apiDocsCoverage")}</p>
            <ul className="mt-4 space-y-3 text-sm leading-6 text-white/68">
              <li>{t("apiDocsCoverageCatalog")}</li>
              <li>{t("apiDocsCoveragePrices")}</li>
              <li>{t("apiDocsCoverageHistory")}</li>
              <li>{t("apiDocsCoverageOperations")}</li>
            </ul>
          </div>
          <div className="rounded-[1.5rem] border border-white/10 bg-white/[0.03] p-5">
            <p className="text-xs uppercase tracking-[0.22em] text-white/45">{t("apiDocsResponseContract")}</p>
            <p className="mt-4 text-sm leading-6 text-white/68">
              {t("apiDocsContract")}
            </p>
            <div className="mt-4 flex flex-wrap gap-2">
              <ApiStatusBadge code={200} label={language === "FR" ? "Succès" : "Success"} />
              <ApiStatusBadge code={400} label="Validation" />
              <ApiStatusBadge code={404} label={language === "FR" ? "Manquant" : "Missing"} />
              <ApiStatusBadge code={500} label={language === "FR" ? "Runtime" : "Runtime"} />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="rounded-[2rem] border border-white/10 bg-[#071123]/72 p-6 shadow-[0_20px_60px_rgba(0,0,0,0.34)] md:p-8">
        <p className="text-[11px] uppercase tracking-[0.24em] text-[#7eb7ff]">{sections[1]?.eyebrow}</p>
        <h2 className="mt-4 text-3xl font-semibold text-white">{sections[1]?.title}</h2>
        <p className="mt-4 max-w-3xl text-base leading-8 text-white/66">{sections[1]?.summary}</p>
        ...
        <div className="mt-8 grid gap-6 xl:grid-cols-[minmax(0,1fr)_26rem]">
          <div className="space-y-4 rounded-[1.75rem] border border-white/10 bg-white/[0.03] p-5">
            ...
            <div>
              <p className="text-xs uppercase tracking-[0.22em] text-white/45">{t("headers")}</p>
...
function EndpointCategoryPanel({
  ...
  const sections = localizeApiDocSections(apiDocSections, language);
  const endpoints = localizeApiDocEndpoints(apiDocEndpoints, language).filter((endpoint) => endpoint.category === categoryId);
...
function StatusCodesPanel() {
  const { language } = usePreferences();
  const sections = localizeApiDocSections(apiDocSections, language);
  const statusReference = localizeApiStatusReference(apiDocStatusReference, language);

  return (
    <section className="rounded-[2rem] border border-white/10 bg-[#071123]/72 p-6 shadow-[0_20px_60px_rgba(0,0,0,0.34)] md:p-8">
      <p className="text-[11px] uppercase tracking-[0.24em] text-[#7eb7ff]">{sections[4]?.eyebrow}</p>
      <h2 className="mt-4 text-3xl font-semibold text-white">{sections[4]?.title}</h2>
      <p className="mt-4 max-w-3xl text-base leading-8 text-white/66">{sections[4]?.summary}</p>
...
function ActivePanel({
  ...
  const { language } = usePreferences();
  const endpoint = localizeApiDocEndpoints(apiDocEndpoints, language).find((item) => item.id === activeId);
